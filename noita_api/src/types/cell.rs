...
impl<T: CellTrait> Cell<T> {
    #[inline]
    #[must_use]
    pub fn cast<K: CellTrait>(self) -> Cell<K> {
        Cell {
            ptr: self.ptr.cast(),
        }
    }
...
impl<T: CellTrait> Copy for Cell<T> {}
impl<T: CellTrait> PartialEq for Cell<T> {
    #[inline]
    fn eq(&self, other: &Self) -> bool {
        self.ptr == other.ptr
    }
}
impl<T: CellTrait> Deref for Cell<T> {
    type Target = CellInner<T>;
    #[inline]
    fn deref(&self) -> &Self::Target {
        &self.ptr
    }
}
impl<T: CellTrait> DerefMut for Cell<T> {
    #[inline]
